...
```toml
[dependencies]
terrace-config = { git = "https://github.com/TimSchoenle/terrace-config", tag = "v0.12.1" }
```
...
```toml
terrace-config = { git = "…", tag = "v0.12.1", features = ["reload", "explain"] }
```
...
```toml
terrace-config = { git = "…", tag = "v0.12.1", default-features = false, features = ["loader"] }
```
