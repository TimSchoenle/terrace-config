...
Everything above is about an *image*. Inside a cluster nothing holds an image: a Kyverno policy, a
validating admission webhook and an init container all hold an object — a `ConfigMap` about to be
mounted, a pod about to start — and the object says nothing about which contract governs it. The
chart that rendered it and the build that produced the image live in different repositories and
never see each other.

So the protocol has a second half, and it is not the first half restated, because Kubernetes will
not accept the same values. A label value is at most 63 bytes and must match
`(([A-Za-z0-9][-A-Za-z0-9_.]*)?[A-Za-z0-9])?`: `PORTFOLIO_` fails on the trailing underscore,
`/config/contract.json` on the slashes, and the media type on both a slash and a `+`. Annotation
values are unconstrained. That forces the split rather than suggesting it:

> **Labels are what you select on. Annotations are what you read.**

`Contract::kube_metadata` renders the block, and `--format kube` prints it:

```yaml
{{- /* on the ConfigMap holding the document */}}
metadata:
  labels:
...
| Key | Kind | What it is for |
|---|---|---|
| `dev.terrace.config/contract-version` | label | The one selectable fact: this object participates, in this version of the protocol. |
| `dev.terrace.config/images` | annotation | Every image that reads this document, comma-separated, **digest-pinned**, in declaration order. |
| `dev.terrace.config/document-key` | annotation | Which key inside `data` is the configuration document. |
| `dev.terrace.config/format` | annotation | Which parser reads it — `toml` today. |

**The version is the entire label surface**, and that is what makes a policy possible at all:
...
A policy written that way never has to enumerate the objects it governs, so it keeps working when
a chart adds a second `ConfigMap`.

There is deliberately no `prefix` and no `contract-path` here. Both are facts about an *image*, and
the image already carries them in its config blob; a copy on an object rendered by a chart is a
second spelling that no build-time check can compare against the first — which is exactly the
drift `verify_labels` catches on the image side, reintroduced where nothing can catch it. There is
no `app` label either, and that one is refused by the design rather than by the character rule: a
document may be read by several images, so the honest field is a list, and every separator a list
needs is outside the label character class. Anything per-image or multi-valued is an annotation.

Two objects get stamped and they are not stamped identically. The document object gets the label
and all three annotations; the **pod template** gets the label and `images` alone, because
`document-key` and `format` describe a document and a pod is not one. The pod is stamped at all so
that an admission webhook — which usually sees only the pod — can tell that the pod is this
protocol's business and find the images to check, without walking ownership references back to
whatever rendered the `ConfigMap`.

`kube::Pairing` is what a cluster-side actor runs. It takes the contract, the running image's
digest-pinned reference and its `config.Labels`, and the mounted object's `metadata`, and asserts
that all of it describes one configuration surface:

```rust,ignore
Pairing::new(&contract, image, &image_labels, &labels, &annotations).check()?;
```

Five checks: the object's version is this contract's; the image's own three labels agree with the
contract — `Contract::verify_labels`, reused rather than reimplemented; the object's version equals
the image's; the running image is a **member** of `images`, because one document may be read by
several; and the document key and format are present and well-formed. Each failure names both
sides and what to do about it.

Digest-pinned is a requirement rather than a preference, and it is the same reasoning that attaches
the contract to a digest rather than to a tag: a tag can be moved after the object was stamped, by
somebody who never touched the chart, and the pairing would still pass against a binary reading
different keys. This crate cannot supply the value — a contract deliberately carries no digest —
so it is passed in by whatever renders the object, and what this crate does is name the annotation,
check the value, and refuse a reference that cannot bear the weight.
