...
//!
//! The Markdown renderings have no use for this: a row is one full path and nothing else. The two
//! renderings that write a *file* have nothing but — a TOML file groups its keys under
//! `[github]` headers, and a JSON Schema nests one `properties` object per level. One walk shared
//! between them, so the two cannot disagree about which keys belong to which table.

use crate::document::Key;

/// One level of the configuration: the keys directly under a path, and the levels below it.
///
/// Order is first-appearance order at every level, which is the order the producer described them
/// in. It is not necessarily contiguous: a document merged from two roots can put a second root's
/// `csp.*` keys behind a first root's `github.*` ones, and a TOML file cannot open the `[csp]`
/// table twice. Grouping here is what makes that case render at all.
pub struct Node<'a> {
...
impl<'a> Node<'a> {
    ...
    ///
    /// Split on `.`, because that is what the loader splits on: a key whose *name* contains a dot
    ...
    pub fn of(keys: impl IntoIterator<Item = &'a Key>) -> Self {
    ...
    /// described beside it. TOML cannot express both, so the renderer has to know.
    pub fn opens(&self, segment: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn a_name_is_the_last_segment() {
        assert_eq!(name("github.token"), "token");
        assert_eq!(name("dist_dir"), "dist_dir");
    }
}
