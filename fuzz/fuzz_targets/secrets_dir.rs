//! Shim over [`terrace_config_fuzz::oracle::secrets_dir`], where the oracle and its documentation live.
//!
...
#[cfg(fuzzing)]
libfuzzer_sys::fuzz_target!(|data: &str| terrace_config_fuzz::oracle::secrets_dir::check(data));

#[cfg(not(fuzzing))]
fn main() {
    eprintln!(
        "built without --cfg fuzzing; run this through `cargo +nightly fuzz run secrets_dir`          or replay the corpus with `cargo test`"
    );
}
