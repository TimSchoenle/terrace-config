//! Shim over [`terrace_config_fuzz::oracle::toml_layers`], where the oracle and its documentation live.
//!
...
#[cfg(fuzzing)]
libfuzzer_sys::fuzz_target!(|data: &str| terrace_config_fuzz::oracle::toml_layers::check(data));

#[cfg(not(fuzzing))]
fn main() {
    eprintln!(
        "built without --cfg fuzzing; run this through `cargo +nightly fuzz run toml_layers`          or replay the corpus with `cargo test`"
    );
}
