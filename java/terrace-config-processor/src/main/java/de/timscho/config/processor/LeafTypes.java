...
final class LeafTypes {
    ...
    static boolean isLeaf(final TypeMirror type) {
    ...
    static boolean isNumeric(final TypeMirror type) {
        if (!isLeaf(type)) {
            return false;
        }
        final String name = type.getKind().isPrimitive() ? boxedName(type) : type.toString();
        return switch (name) {
    ...
    private static String boxedName(final TypeMirror type) {
