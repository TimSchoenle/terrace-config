...
final class TypeNames {
    ...
    static String simplify(final String qualified) {
        final StringBuilder out = new StringBuilder();
        final StringBuilder segment = new StringBuilder();
        for (int i = 0; i <= qualified.length(); i++) {
            final char c = i < qualified.length() ? qualified.charAt(i) : 0;
            final boolean boundary = i == qualified.length() || c == '<' || c == '>' || c == ',' || c == ' ';
            if (boundary) {
    ...
    private static String lastSegment(final String dotted) {
        final int idx = dotted.lastIndexOf('.');
