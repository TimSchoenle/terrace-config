...
```toml
[dependencies]
terrace-config = { git = "https://github.com/TimSchoenle/terrace-config", tag = "v0.15.0" }
```
...
```toml
terrace-config = { git = "…", tag = "v0.15.0", features = ["reload", "explain"] }
```
...
```toml
terrace-config = { git = "…", tag = "v0.15.0", default-features = false, features = ["loader"] }
```
