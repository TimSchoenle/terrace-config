//! Shim over [`terrace_config_fuzz::oracle::env_load`], where the oracle and its documentation live.
//!
...
#[cfg(fuzzing)]
libfuzzer_sys::fuzz_target!(|data: &str| terrace_config_fuzz::oracle::env_load::check(data));

#[cfg(not(fuzzing))]
fn main() {
    eprintln!(
        "built without --cfg fuzzing; run this through `cargo +nightly fuzz run env_load`          or replay the corpus with `cargo test`"
    );
}
