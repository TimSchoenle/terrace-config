//! Shim over [`terrace_config_fuzz::oracle::schema`], where the oracle and its documentation live.
//!
...
#[cfg(fuzzing)]
libfuzzer_sys::fuzz_target!(|data: &str| terrace_config_fuzz::oracle::schema::check(data));

#[cfg(not(fuzzing))]
fn main() {
    eprintln!(
        "built without --cfg fuzzing; run this through `cargo +nightly fuzz run schema` \
